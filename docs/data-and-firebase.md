...
**Refresh (restock):** [`refreshPantryItem`](../src/lib/firestore.ts) keeps the item’s identity (names, normalized name, category, staple flag, catalog link) but clears quantity, unit, note, `expiresOn`, `isOpened`, and `imageUrl`, sets `addedAt` to now, then deletes the previous Storage image if one existed.
